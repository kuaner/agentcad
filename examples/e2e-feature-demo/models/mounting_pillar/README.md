...
```bash
agentcad precheck mounting_pillar     # 1. solve design.json statically
# write part.py once precheck is green
agentcad build mounting_pillar        # 2. generate STEP/STL
agentcad measure mounting_pillar      # 3. measure geometry stats
agentcad validate mounting_pillar     # 4. run all checks (auto-renders SVGs)
agentcad review mounting_pillar       # 5. pre-delivery checklist + relations matrix
agentcad deliver mounting_pillar      # 6. write delivery manifest
```
