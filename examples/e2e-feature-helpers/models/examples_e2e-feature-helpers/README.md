...
```bash
agentcad precheck examples_e2e-feature-helpers     # 1. solve design.json statically
# write part.py once precheck is green
agentcad build examples_e2e-feature-helpers        # 2. generate STEP/STL
agentcad measure examples_e2e-feature-helpers      # 3. measure geometry stats
agentcad validate examples_e2e-feature-helpers     # 4. run all checks (auto-renders SVGs)
agentcad review examples_e2e-feature-helpers       # 5. pre-delivery checklist + relations matrix
agentcad deliver examples_e2e-feature-helpers      # 6. write delivery manifest
```
