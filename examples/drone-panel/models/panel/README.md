...
```bash
agentcad precheck panel     # 1. solve design.json statically
# write part.py once precheck is green
agentcad build panel        # 2. generate STEP/STL
agentcad measure panel      # 3. measure geometry stats
agentcad validate panel     # 4. run all checks (auto-renders SVGs)
agentcad review panel       # 5. pre-delivery checklist + relations matrix
agentcad deliver panel      # 6. write delivery manifest
```
