...
```bash
agentcad precheck bracket     # 1. solve design.json statically
# write part.py once precheck is green
agentcad build bracket        # 2. generate STEP/STL
agentcad measure bracket      # 3. measure geometry stats
agentcad validate bracket     # 4. run all checks (auto-renders SVGs)
agentcad review bracket       # 5. pre-delivery checklist + relations matrix
agentcad deliver bracket      # 6. write delivery manifest
```
