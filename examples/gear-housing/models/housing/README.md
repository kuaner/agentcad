...
```bash
agentcad precheck housing     # 1. solve design.json statically
# write part.py once precheck is green
agentcad build housing        # 2. generate STEP/STL
agentcad measure housing      # 3. measure geometry stats
agentcad validate housing     # 4. run all checks (auto-renders SVGs)
agentcad review housing       # 5. pre-delivery checklist + relations matrix
agentcad deliver housing      # 6. write delivery manifest
```
