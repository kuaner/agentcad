...
```bash
agentcad precheck coupling     # 1. solve design.json statically
# write part.py once precheck is green
agentcad build coupling        # 2. generate STEP/STL
agentcad measure coupling      # 3. measure geometry stats
agentcad validate coupling     # 4. run all checks (auto-renders SVGs)
agentcad review coupling       # 5. pre-delivery checklist + relations matrix
agentcad deliver coupling      # 6. write delivery manifest
```
