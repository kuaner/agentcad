...
class SectionCache:
    ...
    Caches section segments and analyses keyed by (axis, position).
    ...
    def __init__(self) -> None:
        self._segments: dict[tuple[int, float], list[Segment2D]] = {}
        self._analysis: dict[tuple[int, float], dict] = {}

    def segments(self, triangles: list[Triangle], axis: int, position: float) -> list[Segment2D]:
        key = (axis, round(position, 5))
